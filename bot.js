...
client.on('message', async message => {
  if(message.content.startsWith(prefix + "order")) {
    await message.channel.send("** ❓  نوع البوت الذي تريده**").then(e => {
...
e.delete();
     message.channel.send('** كم المدة اللتي تريده ان يبقى شغالا فيها او الى الابد ❓**').then(m => {
let chaMd = message.channel.awaitMessages(filter, { max: 1, time: 40000, errors: ['time'] })
.then(co => {
  md = co.first().content
        co.first().delete()
        m.delete();
message.channel.send('**هل تريد التعديل عليه في وقت لاحق ام ابقائه كما هو جتى انتهاء صلاحيته ❓ **').then(ms => {
...
.setDescription(`**  نوع البوت :question:  : \n ${lan}\nمدة التشغيل :link: :\n ${md} \nتعديل :question: :\n ${br}  \nتم التقديم بواسطة  : <@${message.author.id}> **`)  
